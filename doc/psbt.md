# PSBT Howto for Faithcoin Core

Since Faithcoin Core 0.17, an RPC interface exists for Partially Signed Faithcoin
...
PSBT is an interchange format for Faithcoin transactions that are not fully signed
...
#### Multisig with multiple Faithcoin Core instances

Alice, Bob, and Carol want to create a 2-of-3 multisig address. They're all using
Faithcoin Core. We assume their wallets only contain the multisig funds. In case
they also have a personal wallet, this can be accomplished through the
multiwallet feature - possibly resulting in a need to add `-rpcwallet=name` to
the command line in case `arvcoin-cli` is used.

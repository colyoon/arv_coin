Faithcoin Core version 0.13.2 is now available from:
  ...
  <https://download.arvcoin.org/arvcoin-0.13.2.1/>
...
updates will be released anymore. Without security updates, using a arvcoin
...
This allows running arvcoind without having to do any manual configuration.
...
`arvcoin.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
caching. A sample config for apache2 could look like:
    ...
    <Location /arvcoinrpc>
        ...
        # optional bypass arvcoind rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< arvcoinrpc:<password>
    </Location>
...
Restarting arvcoind will no longer clear out the list of banned peers; instead
...
- Adding `dbcache=100` in arvcoin.conf
...
For example:
    ...
    $ src/arvcoin-cli -stdin walletpassphrase
    mysecretcode
    120
...
- `arvcoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `arvcoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of arvcoins they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of arvcoins being spent is exactly
...
- `arvcoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `arvcoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting

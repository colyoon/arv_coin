Faithcoin Core version *0.15.0* is now available from:
  ...
  <https://download.arvcoin.org/arvcoin-0.15.0/>
...
Faithcoin Core 0.15.0 contains the following changes to the RPC interface and `arvcoin-cli` for multi-wallet:

* When running Faithcoin Core with a single wallet, there are **no** changes to the RPC interface or `arvcoin-cli`. All RPC calls and `arvcoin-cli` commands continue to work as before.
* When running Faithcoin Core with multi-wallet, all *node-level* RPC methods continue to work as before. HTTP RPC requests should be send to the normal `<RPC IP address>:<RPC port>/` endpoint, and `arvcoin-cli` commands should be run as before. A *node-level* RPC method is any method which does not require access to the wallet.
* When running Faithcoin Core with multi-wallet, *wallet-level* RPC methods must specify the wallet for which they're intended in every request. HTTP RPC requests should be send to the `<RPC IP address>:<RPC port>/wallet/<wallet name>/` endpoint, for example `127.0.0.1:9332/wallet/wallet1.dat/`. `arvcoin-cli` commands should be run with a `-rpcwallet` option, for example `arvcoin-cli -rpcwallet=wallet1.dat getbalance`.
...
- #9667 `fa625b0` Document CWalletTx::mapValue entries and remove erase of nonexistent "version" entry (ryanofsky)

Faithcoin Core version *0.15.1* is now available from:

  <https://download.arvcoin.org/arvcoin-0.15.1/>
...
A number of changes to the way Faithcoin Core deals with peer connections and invalid blocks
...
The minimum relay fee `-minrelayfee` has been lowered to 0.01 autonomousvehicles per kB (0.00001 ARV). This is to prepare for dropping the minimum transaction fee to 0.1 autonomousvehicles per kB (0.0001 ARV) in the next release.

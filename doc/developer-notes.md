...
- [Development guidelines](#development-guidelines)
    - [General Faithcoin Core](#general-arvcoin-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
Faithcoin Core uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
Run with the `-testnet` option to run with "play arvcoins" on the test network, if you
...
Faithcoin Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/arvcoin-project/arvcoin/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_arvcoin
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_arvcoin --log_level=test_suite
$ valgrind -v --leak-check=full src/arvcoind -printtoconsole
```
...
# A coverage report will now be accessible at `./test_arvcoin.coverage/index.html`.
```
...
To profile a running arvcoind process for 60 seconds, you could use an
invocation of `perf record` like this:

```sh
$ perf record \
    -g --call-graph dwarf --per-thread -F 140 \
    -p `pgrep arvcoind` -- sleep 60
```
...
Faithcoin Core can be compiled with various "sanitizers" enabled, which add
...
- ThreadSocketHandler : Sends/Receives data from peers on port 9667.
...
file descriptors it will cause problems with Faithcoin's `select()` loop, because
...
would be to revert the upstream fix before applying the updates to Faithcoin's
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `arvcoin-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `arvcoin-cli` and the GUI debug console use this table to determine how to
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of Faithcoin Core, the wallet was always
